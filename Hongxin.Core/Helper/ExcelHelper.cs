...
namespace Hongxin.Core.Helper
{
    ...
    public class ExcelHelper
    {
        ...
        //设置一个单元格的属性   字体，   大小，颜色   ，对齐方式
        {
            ...
            if(!string.IsNullOrEmpty(name)) ws.Range[ws.Cells[Startx, Starty], ws.Cells[Endx, Endy]].Font.Name = name;
            if (size != 0) ws.Range[ws.Cells[Startx, Starty], ws.Cells[Endx, Endy]].Font.Size = size;
        ...
        {
            ws.Range[ws.Cells[x1, y1], ws.Cells[x2, y2]].RowHeight = rowHeight;
        }

        public void SetCellProperty_Table(Microsoft.Office.Interop.Excel.Worksheet ws, int x1, int y1, int x2, int y2)
        {
            var range = ws.Range[ws.Cells[x1, y1], ws.Cells[x2, y2]];
            var borders = ws.Range[ws.Cells[x1, y1], ws.Cells[x2, y2]].Borders;
            borders.Item(Microsoft.Office.Interop.Excel.XlBordersIndex.xlInsideVertical);
            borders.LineStyle = Microsoft.Office.Interop.Excel.XlLineStyle.xlContinuous;
            borders.Weight = Microsoft.Office.Interop.Excel.XlBorderWeight.xlThin;
            //borders.ColorIndex = System.Drawing.Color.Black.ToArgb();
            range.BorderAround(Microsoft.Office.Interop.Excel.XlLineStyle.xlContinuous, XlBorderWeight.xlThin,
                                          XlColorIndex.xlColorIndexAutomatic, System.Drawing.Color.Black.ToArgb()); 
        }
        public void SetCellProperty_Border_Bottom(Microsoft.Office.Interop.Excel.Worksheet ws, int x1, int y1, int x2, int y2)
        {
            var range = ws.Range[ws.Cells[x1, y1], ws.Cells[x2, y2]];
            var borders = ws.Range[ws.Cells[x1, y1], ws.Cells[x2, y2]].Borders;
            var bottoms = borders.Item(Microsoft.Office.Interop.Excel.XlBordersIndex.xlEdgeBottom);
            bottoms.LineStyle = Microsoft.Office.Interop.Excel.XlLineStyle.xlContinuous;
            bottoms.Weight = Microsoft.Office.Interop.Excel.XlBorderWeight.xlThin;
            //range.BorderAround(Microsoft.Office.Interop.Excel.XlLineStyle.xlContinuous, XlBorderWeight.xlThin,
            //                              XlColorIndex.xlColorIndexNone, null); 
        }
        public void SetCellProperty_WrapText(Microsoft.Office.Interop.Excel.Worksheet ws, int x1, int y1, int x2, int y2, bool isTrue)
        {
            ws.Range[ws.Cells[x1, y1], ws.Cells[x2, y2]].WrapText = isTrue;
        }
